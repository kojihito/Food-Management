...
function thanhtoanpage(option,product) {
    ...
    // === TỰ ĐỘNG ĐIỀN THÔNG TIN: TÊN + SĐT + ĐỊA CHỈ ===
    function loadSavedAddress() {
        ...
        const streetInput = document.getElementById('street');
            if (streetInput && currentUser.address) {
                streetInput.value = currentUser.address;
        }
    }

    /*
    // GỌI NGAY KHI MỞ TRANG THANH TOÁN
    loadSavedAddress();
    */

    setTimeout(() => {
        loadSavedAddress();       // tên + sđt
        loadDetailedAddress();    // số nhà + phường + thành phố
    }, 300);
...
function xulyDathang(product) {
    ...
    if (giaotannoi && giaotannoi.classList.contains("active")) {
        ...
        streetVal = streetEl ? streetEl.value.trim() : "";
        wardVal = wardEl ? wardEl.value.trim() : "";
        diachinhan = streetVal && wardVal ? `${streetVal}, ${wardVal}` : (streetVal || "");
    ...
    if (giaongay && giaongay.checked) {
        ...
        const choise = document.querySelector(".choise-time");
        thoigiangiao = choise ? choise.value : "";
    } else {
        thoigiangiao = "";
    }
    ...
    // Validation theo hình thức
    if (tudenlay && tudenlay.classList.contains("active")) {
        // Tự đến lấy: cần chọn chi nhánh
        if (!diachinhan) {
            toast({ title: 'Chú ý', message: 'Vui lòng chọn chi nhánh để lấy hàng !', type: 'warning', duration: 4000 });
            return;
        }
    } else if (giaotannoi && giaotannoi.classList.contains("active")) {
        // Giao tận nơi: cần thời gian, tên, sđt, đường và phường
        if (!giaongay && !giaovaogio) {
            toast({ title: 'Chú ý', message: 'Vui lòng chọn thời gian giao hàng !', type: 'warning', duration: 4000 });
            return;
        }
        if (giaovaogio && giaovaogio.checked && thoigiangiao === "") {
            toast({ title: 'Chú ý', message: 'Vui lòng chọn thời gian giao hàng !', type: 'warning', duration: 4000 });
            return;
        }
        if (!streetVal || !wardVal) {
            toast({ title: 'Chú ý', message: 'Vui lòng nhập đầy đủ địa chỉ (số nhà/tên đường và phường) !', type: 'warning', duration: 4000 });
            return;
        }
    }

    // Kiểm tra tên và sđt luôn bắt buộc
    if (tennguoinhan == "" || sdtnhan == "") {
        toast({ title: 'Chú ý', message: 'Vui lòng nhập đầy đủ thông tin (tên và số điện thoại) !', type: 'warning', duration: 4000 });
        return;
    }

    // Nếu vượt qua hết validation -> tạo đơn
    let donhang = {
        id: madon,
    ...
    //
    saveDetailedAddress(); // Lưu địa chỉ chi tiết vào tài khoản
    //
    toast({ title: 'Thành công', message: 'Đặt hàng thành công !', type: 'success', duration: 1000 });
    setTimeout(() => {
        window.location.href = "./index.html"; // Sửa thành đường dẫn tương đối
    }, 1500);
}
